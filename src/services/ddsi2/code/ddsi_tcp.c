...
static ddsi_tcp_conn_t ddsi_tcp_new_conn (os_socket, c_bool, os_sockaddr_storage *);
...
static unsigned short get_socket_port (os_socket socket)
{
  ...
  {
    int err = os_getErrno();
    NN_ERROR1 ("ddsi_tcp_get_socket_port: getsockname errno %d\n", err);
    return 0;
  }
...
static void ddsi_tcp_conn_connect (ddsi_tcp_conn_t conn, const struct msghdr * msg)
{
  ...
  if (sock != Q_INVALID_SOCKET)
  {
    ...
    os_sockSetNonBlocking (conn->m_sock, OS_TRUE);
...
static os_ssize_t ddsi_tcp_conn_read_plain (ddsi_tcp_conn_t tcp, void * buf, os_size_t len, int * err)
{
  os_ssize_t ret = recv (tcp->m_sock, buf, len, 0);
...
static c_bool ddsi_tcp_select (os_socket sock, c_bool read, os_size_t pos)
{
  int ret;
  fd_set fds;
  os_duration timeout;
  ...
  os_int64 tval = read ? config.tcp_read_timeout : config.tcp_write_timeout;

  FD_ZERO (&fds);
  FD_SET (sock, &fds);
  timeout = (os_duration)tval;
  ...
  if (ret <= 0)
  {
    nn_log
    (
      LC_WARNING, "%s abandoning %s on blocking socket %d after %"PA_PRIuSIZE" bytes\n",
...
static os_ssize_t ddsi_tcp_conn_read (ddsi_tran_conn_t conn, unsigned char * buf, os_size_t len, nn_locator_t *srcloc)
{
  ddsi_tcp_conn_t tcp = (ddsi_tcp_conn_t) conn;
  os_ssize_t (*rd) (ddsi_tcp_conn_t, void *, os_size_t, int * err) = ddsi_tcp_conn_read_plain;
  os_size_t pos = 0;
  os_ssize_t n;
  int err;
...
static os_ssize_t ddsi_tcp_conn_write_plain (ddsi_tcp_conn_t conn, const void * buf, os_size_t len, int * err)
{
  os_ssize_t ret;
  int sendflags = 0;
...
static os_ssize_t ddsi_tcp_block_write
(
  os_ssize_t (*wr) (ddsi_tcp_conn_t, const void *, os_size_t, int *),
...
)
{
  ...
  return (pos == sz) ? (os_ssize_t) pos : -1;
}
...
static os_ssize_t ddsi_tcp_conn_write (ddsi_tran_conn_t base, const nn_locator_t *dst, size_t niov, const ddsi_iovec_t *iov, os_uint32 flags)
{
#ifdef DDSI_INCLUDE_SSL
...
#endif
  {
    ...
    else
    {
      ...
      piecewise = (ret > 0 && (os_size_t) ret < len);
    }
  }

  if (piecewise)
  {
    os_ssize_t (*wr) (ddsi_tcp_conn_t, const void *, os_size_t, int *) = ddsi_tcp_conn_write_plain;
    int i = 0;
#ifdef DDSI_INCLUDE_SSL
    ...
    while (ret >= (os_ssize_t) msg.msg_iov[i].iov_len)
    {
      ret -= (os_ssize_t) msg.msg_iov[i++].iov_len;
    }
    assert (i < (int) msg.msg_iovlen);
    ret = ddsi_tcp_block_write (wr, conn, (const char *) msg.msg_iov[i].iov_base + ret, msg.msg_iov[i].iov_len - (os_size_t) ret);
  ...
  return ((os_size_t) ret == len) ? ret : -1;
}
...
static c_bool ddsi_tcp_supports (os_int32 kind)
{
  return
  (
...
static ddsi_tcp_conn_t ddsi_tcp_new_conn (os_socket sock, c_bool server, os_sockaddr_storage * peer)
{
...
static ddsi_tran_listener_t ddsi_tcp_create_listener (int port, ddsi_tran_qos_t qos)
{
  ...
  if (sock != Q_INVALID_SOCKET)
  {
    ...
    {
      int err = os_getErrno ();
      NN_ERROR1 ("ddsi_tcp_create_listener: getsockname errno %d\n", err);
      ddsi_tcp_sock_free (sock, NULL);
      os_free (tl);
      return NULL;
    }
