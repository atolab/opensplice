...
int spdp_write (struct participant *pp)
{
  ...
  nn_plist_addtomsg (mpayload, &ps, ~(os_uint64)0, 0);
...
int spdp_dispose_unregister (struct participant *pp)
{
  ...
  ps.participant_guid = pp->e.guid;
  nn_plist_addtomsg (mpayload, &ps, ~(os_uint64)0, ~(os_uint64)0);
  nn_xmsg_addpar_sentinel (mpayload);
...
#undef UINT64_CONST
  ...
  os_uint32 i = (os_uint32) ((os_uint64) tnow.v >> 32), j = (os_uint32) tnow.v;
  os_uint64 m = 0;
  m += (a + cs[0]) * (b + cs[1]);
  m += (c + cs[2]) * (d + cs[3]);
  m += (e + cs[4]) * (f + cs[5]);
...
static void make_participants_dependent_on_ddsi2 (const nn_guid_t *ddsi2guid)
{
  ...
  if ((d2pp = ephash_lookup_proxy_participant_guid (ddsi2guid)) == NULL)
    return;
  d2pp_lease = pa_ldvoidp (&d2pp->lease);
  ephash_enum_proxy_participant_init (&it);
  while ((pp = ephash_enum_proxy_participant_next (&it)) != NULL)
  {
...
static void handle_SPDP_alive (const struct receiver_state *rst, const nn_plist_t *datap)
{
  ...
  if ((proxypp = ephash_lookup_proxy_participant_guid (&datap->participant_guid)) != NULL)
  {
    ...
    TRACE ((" (known)"));
    lease_renew (pa_ldvoidp (&proxypp->lease), now_et ());
...
static void handle_SPDP (const struct receiver_state *rst, unsigned statusinfo, const void *vdata, unsigned len)
{
  ...
  else
  {
    ...
    src.bufsz = len - 4;
    if (nn_plist_init_frommsg (&decoded_data, NULL, ~(os_uint64)0, ~(os_uint64)0, &src) < 0)
    {
   ...
   const nn_xqos_t *xqos, struct addrset *as)
{
  ...
  else
  {
    ...
    qosdiff = nn_xqos_delta (xqos, defqos, ~(os_uint64)0);
    if (config.explicitly_publish_qos_set_to_default)
      qosdiff |= ~QP_UNRECOGNIZED_INCOMPATIBLE_MASK;
...
int sedp_write_topic (struct participant *pp, const struct nn_plist *datap)
{
  ...
  void *payload_blob;
  size_t payload_sz;
  os_uint32 topic_name_sz;
  os_uint32 topic_name_sz_BE;
  os_uint64 delta;
  ...
  delta = nn_xqos_delta (&datap->qos, &gv.default_xqos_tp, ~(os_uint64)0);
  if (config.explicitly_publish_qos_set_to_default)
    delta |= ~QP_UNRECOGNIZED_INCOMPATIBLE_MASK;
  nn_plist_addtomsg (mpayload, datap, ~(os_uint64)0, delta);
  nn_xmsg_addpar_sentinel (mpayload);
  ...
  topic_name_sz = (os_uint32) strlen (datap->qos.topic_name) + 1;
...
int sedp_write_cm_publisher (const struct nn_plist *datap, int alive)
{
  ...
  {
    delta = nn_xqos_delta (&datap->qos, &gv.default_xqos_pub, ~(os_uint64)0);
    if (!config.explicitly_publish_qos_set_to_default)
      delta |= ~QP_UNRECOGNIZED_INCOMPATIBLE_MASK;
  }
  nn_plist_addtomsg (mpayload, datap, ~(os_uint64)0, delta);
...
int sedp_write_cm_subscriber (const struct nn_plist *datap, int alive)
{
  ...
  {
    delta = nn_xqos_delta (&datap->qos, &gv.default_xqos_sub, ~(os_uint64)0);
    if (!config.explicitly_publish_qos_set_to_default)
      delta |= ~QP_UNRECOGNIZED_INCOMPATIBLE_MASK;
  }
  nn_plist_addtomsg (mpayload, datap, ~(os_uint64)0, delta);
...
int builtins_dqueue_handler (const struct nn_rsample_info *sampleinfo, const struct nn_rdata *fragchain, UNUSED_ARG (const nn_guid_t *rdguid), UNUSED_ARG (void *qarg))
{
  ...
  unsigned char *datap;
  int needs_free;
  os_uint32 datasz = sampleinfo->size;
