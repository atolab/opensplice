...
DU (uf_transport_selector);
DU (uf_many_sockets_mode);
#undef DU
...
PF (pf_transport_selector);
PF (pf_many_sockets_mode);
#undef PF
...
)
{
     ...
     the poor framework */
  {
    ...
    if (!ok1)
      NN_ERROR0 ("config: invalid combination of Transport, IPv6, TCP\n");
    ok = ok && ok1;
