...
static os_ssize_t ddsi_raweth_conn_read (ddsi_tran_conn_t conn, unsigned char * buf, os_size_t len, nn_locator_t *srcloc)
{
  int err;
  os_ssize_t ret;
...
static os_ssize_t ddsi_raweth_conn_write (ddsi_tran_conn_t conn, const nn_locator_t *dst, size_t niov, const ddsi_iovec_t *iov, os_uint32 flags)
{
  ...
  if (ret == -1)
  {
    switch (err)
    {
      ...
      case os_sockEHOSTUNREACH:
        ...
        break;
      default:
        NN_ERROR1("ddsi_raweth_conn_write failed with error code %d", err);
    }
  }
  return ret;
}
...
static c_bool ddsi_raweth_supports (os_int32 kind)
{
...
static ddsi_tran_conn_t ddsi_raweth_create_conn (os_uint32 port, ddsi_tran_qos_t qos)
{
  os_socket sock;
  int rc;
  ...
  {
    NN_ERROR3("ddsi_raweth_create_conn %s port %u - using port number as ethernet type, %u won't do\n", mcast ? "multicast" : "unicast", port, port);
    return NULL;
  }
  ...
  {
    rc = os_getErrno();
    NN_ERROR3("ddsi_raweth_create_conn %s port %u failed ... errno = %d\n", mcast ? "multicast" : "unicast", port, rc);
    return NULL;
  }
  ...
  if (bind(sock, (struct sockaddr *)&addr, sizeof(addr)) == -1)
  {
    rc = os_getErrno();
    close(sock);
    NN_ERROR3("ddsi_raweth_create_conn %s bind port %u failed ... errno = %d\n", mcast ? "multicast" : "unicast", port, rc);
    return NULL;
  }
